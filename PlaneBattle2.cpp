...
public:
	...
	bool login_in(const string& usename, const string& password);
	Login(int num);
	bool log(int num0);
	~Login();
	void setscore(int num, int score0);
};
void Login::set_password(string& usename, string& password)
{
	bool again = true;
	bool tmp1 = true;
	bool tmp2 = true;
	...
	do
	{
		...
		{
			if (islower(password.at(i)) || isupper(password.at(i)))
			{
				for (int j = 0; j < password.length(); j++)
				{
					if (isalpha(password.at(j)))
					{
						tmp2 = false;
						break;
					}
...
bool Login::login_in(const string& usename, const string& password)
{
	int logintime = TIME;
...
void Login::set(int num0)
{
	set_password((userp + num0)->name, (userp + num0)->password);
}
bool Login::log(int num0)
{
	 return login_in((userp + num0)->name, (userp + num0)->password);
}
Login::~Login()
{
	delete[] userp;
}
void Login::setscore(int num,int score0)
{
	(userp + num)->score = score0;
}
int main()
{
	...
	int num;
	cin >> num;
	Login log_ = Login(num);
	for (int i = 0; i < num; i++)
	{
		cout << "请输入第" << i + 1 << "玩家的账号信息" << endl;
		log_.set(i);
	}
	cout << "您要登录哪位玩家的账号" << endl;
	cin >> num;
	if (log_.log(num - 1))
	{
		hello();
	...
	{
		cout << "密码错误次数过多，账号自动注销" << endl << "任意键退出";
		while (!_kbhit());
	}
}
